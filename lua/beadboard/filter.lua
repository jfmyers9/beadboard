...
function M.build_args(buf)
	...
	table.insert(args, f.tree and "0" or tostring(cfg.default_limit))
...
function M.describe(buf)
	...
	if f.status then
		table.insert(parts, "status:" .. f.status)
	end
	if f.type then
		table.insert(parts, "type:" .. f.type)
	end
	if f.priority then
		table.insert(parts, "priority:P" .. f.priority)
	end
	if f.assignee then
		table.insert(parts, "assignee:" .. f.assignee)
	end
	if f.label then
		table.insert(parts, "label:" .. f.label)
	end
	if f.label_any then
		table.insert(parts, "label-any:" .. f.label_any)
	end
	if f.created_after then
		table.insert(parts, "after:" .. f.created_after)
	end
	if f.created_before then
		table.insert(parts, "before:" .. f.created_before)
	end
	if f.updated_after then
		table.insert(parts, "updated>:" .. f.updated_after)
	end
	if f.updated_before then
		table.insert(parts, "updated<:" .. f.updated_before)
	end
	if f.show_all then
		table.insert(parts, "all")
	end
	if f.empty_description then
		table.insert(parts, "empty-desc")
	end
	if f.tree then
